...
def delete_product(request, id):
    ...
    product.delete()
    messages.warning(request, f"Product '{product_name}' has been deleted.")
...
def update_product_entry_ajax(request, id):
    ...
    thumbnail = request.POST.get("thumbnail", product.thumbnail or "")
    is_featured = request.POST.get("is_featured")
    if is_featured is not None:
        is_featured = is_featured in ("on", "true", "True", "1")
    else:
        is_featured = product.is_featured
    ...
    product.price = price
    product.save()
    return JsonResponse({"status": "updated", "id": str(product.id)}, status=200)
